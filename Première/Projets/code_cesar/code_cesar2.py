...
os.system("clear")
fichier = input("Entrez le fichier contenant la phrase à coder: ")
clean = open(fichier,"r").read()
pas = int(input("Entrez la valeur de décalage: "))
# formatage de la clé
clef = pas % 26
encode = caesar(clean, clef)
open("phrase_encodee.txt", "w").write(encode)
os.system("clear")
print("Nom du fichier: "+ fichier +"\nMessage de départ: "+ clean + "\nPas: "+ str(pas) + "\nMessage chiffré: "+ encode+"\n---\nLe message encodé est enregistré dans: phrase_encodee.txt")
